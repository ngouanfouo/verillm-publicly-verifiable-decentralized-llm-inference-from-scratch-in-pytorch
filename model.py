...
def build_merkle_level(nodes):
    """Hash adjacent pairs of nodes, duplicating the last if odd, to form the next Merkle level."""
    if not nodes:
        return []
...
def assemble_public_transcript(prover_result, prompt_ids):
    ...
    # Extract components from prover_result
    output_tokens = prover_result['output_tokens']
    leaves = prover_result['leaves']
    step_states = prover_result['step_states']
    ...
    # Assemble the transcript with copies
    transcript = {
        'prompt_ids': copy.copy(prompt_ids),
        'output_tokens': copy.copy(output_tokens),
        'leaves': copy.copy(leaves),
        'tree': tree,
        'root': root,
        'step_states': copy.copy(step_states)
    }
...
def recompute_step_commitment(reexec_state, prior_kv_cache):
    ...
    # Extract the step index from the state
    step_index = reexec_state.get('step_index', 0)
    
    # Create a state dict that matches what the prover would have committed
    # This is essentially the same structure
    state_for_commit = {
        'step_index': step_index,
        'input_token': reexec_state.get('input_token'),
        'next_token': reexec_state.get('next_token'),
        'logits': reexec_state.get('logits'),
        'kv_caches': reexec_state.get('kv_caches'),
        'next_pos': reexec_state.get('next_pos')
    }
    
    # Use commit_decode_step to produce the leaf
    leaf = commit_decode_step(state_for_commit)
...
def aggregate_votes_majority(votes):
    accept_count = sum(1 for v in votes if v.get('vote') is True)
    reject_count = sum(1 for v in votes if v.get('vote') is False)
...
def reward_honest_participants(balances, worker_id, votes, verdict, reward_worker, reward_verifier):
    ...
    # Credit worker if verdict is accept (True)
    if verdict:
        new_balances[worker_id] = new_balances.get(worker_id, 0.0) + reward_worker
        
    # Credit verifiers whose vote matches the final verdict
    for v in votes:
        if v.get('vote') == verdict:
            v_id = v['verifier_id']
            new_balances[v_id] = new_balances.get(v_id, 0.0) + reward_verifier
...
def slash_worker(balances, worker_id, slash_amount):
    new_balances = balances.copy()
    current_balance = new_balances.get(worker_id, 0.0)
